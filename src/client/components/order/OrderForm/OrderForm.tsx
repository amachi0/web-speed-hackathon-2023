...
export const OrderForm: FC<Props> = ({ onSubmit }) => {
  ...
  const handleZipcodeChange: ChangeEventHandler<HTMLInputElement> = async ( event) => {
    ...
    if (zipCode.length == 7) {
      ...
      if (jsonData.status !== 200 || jsonData.results.length === 0) {
        const prefecture = jsonData.results[0].address1;
        const city = jsonData.results[0].address2;
        await formik.setFieldValue('prefecture', prefecture);
